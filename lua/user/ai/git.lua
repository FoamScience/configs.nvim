...
M._fetch_staged_diffs = function()
    local git_command = { "git", "diff", "--cached" }
    local hunks = {}
    require("plenary.job"):new({
        command = git_command[1],
        args = { unpack(git_command, 2) },
        on_exit = function(job, return_val)
            if return_val == 0 then
                local result = job:result()
                if #result > 0 then
                    for _, line in ipairs(result) do
                        table.insert(hunks, line)
                    end
                else
                    print("No staged changes found.")
                end
            else
                print("Error running git diff --cached")
            end
        end,
    }):start()
end
...
M.git_commits_prompt = function()
    ...
    local status, diffs = pcall(M._get_git_diffs)
    if not status or diffs == nil then
        vim.notify("No git diffs found.", vim.log.levels.WARN)
        return nil
    end
...
M.chat_git_commitmsg = function()
    local parser = function(parsed_data)
        if parsed_data ~= nil and type(parsed_data) == "table" then
