...
return {
    ...
    config = function()
        ...
        -- Register commands
        vim.api.nvim_create_user_command("FoamUtListTests", M.FoamUtListTests, {
            desc = "List and select foamUT tests to run",
        })
        vim.api.nvim_create_user_command("FoamUtRunTestAtCursor", M.FoamUtRunTestAtCursor, {
            desc = "Run the foamUT test at cursor position",
        })

        -- Store module globally for potential external access
        _G.FoamUT = M
    end,
}
